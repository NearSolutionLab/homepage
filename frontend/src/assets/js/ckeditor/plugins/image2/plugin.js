...
( function() {
	...
	CKEDITOR.plugins.add( 'image2', {
		...
		onLoad: function() {
            ...
            '.cke_widget_wrapper:hover .cke_image_tools,' +
			...
			'.cke_image_tools{' +
                ...
                'height: 35px;' +
            '}' +
            '.cke_image_tools>span.cke_background_image{' +
                'position: absolute;' +
                'width: 100%;' +
                'height: 100%;' +
                'opacity: 0.8;' +
                'border-radius: 5px;' +
                'box-shadow: 0px 2px 8px 0 rgba(0, 0, 0, 0.48);' +
                'background-color: rgb(14, 27, 38, 0.8);' +
            '}' +
            '.cke_image_tools>span.cke_tool_list{' +
                'position: absolute;' +
            '}' +

            '.cke_widget_wrapper > img.cke_image_border{' +
            ...
            '.cke_image_tools a{' +
                ...
                'cursor: pointer;' +
            '}' +
            '.cke_image_tools a:first-of-type{' +
                'margin-left: 40px;' +
            '}' +
            '.cke_image_tools a:hover>img{' +
                'opacity: 1;' +
            '}' +
	        '.cke_image_tools a>img{' +
	            'width: 14px;' +
			    'height: 14px;' +
			    'object-fit: contain;' +
                'outline: none;' +
		...
		afterInit: function( editor ) {
			...
			var setAutoFileSelection = function() {
			    if (!CKEDITOR.dialog || !CKEDITOR.dialog.getCurrent()) {
			        return null;
			    }
			    var inputFile = $(CKEDITOR.dialog.getCurrent().parts.dialog.$).find('iframe.cke_dialog_ui_input_file').contents().find('input[type="file"]').focus().click();
			    return inputFile && inputFile.length > 0;
			}
			editor.on('loaded', function(){
				$(editor.element.$).next().click('.cke_button__image', function(){
					if (!setAutoFileSelection()) {
						setTimeout(function(){
							setAutoFileSelection();
						},100);
					}
				});
			});
    ...
    // Defines all features related to image custom tools.
    //
    // @param {CKEDITOR.plugins.widget} widget
    function setupTools( widget ) {
        ...
        tools = widget.tools = new CKEDITOR.dom.element.createFromHtml(
                '<span class="cke_image_tools">' +
                    '<span class="cke_background_image"></span>' +
                    '<span class="cke_tool_list">' +
                        '<a data-cke-image-tool-id="deflate" title="' + editor.lang.image2.deflate + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/minimize.svg" class="minimize"></a>' +
                        '<a data-cke-image-tool-id="optimize" title="' + editor.lang.image2.optimize + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/full.svg" class="full"></a>' +
                        '<a data-cke-image-tool-id="origin" title="' + editor.lang.image2.origin + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/original.svg" class="original"></a>' +
                        '<a data-cke-image-tool-id="border" title="' + editor.lang.image2.border + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/border.svg" class="border"></a>' +
//                        '<a data-cke-image-tool-id="edit" title="' + editor.lang.image2.edit + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/edit.svg" class="edit"></a>' +
                        '<a data-cke-image-tool-id="remove" title="' + editor.lang.image2.remove + '"><img src="' + CKEDITOR.basePath  + 'plugins/image2/icons/delete.svg" class="delete"></a>' +
                    '</span>' +
                '</span>',doc);
        ...
        if ( !widget.inline ) {
            var imageOrLink = widget.parts.link || widget.parts.image,
                ...
                toolsWrapper = doc.createElement( 'span' );

            toolsWrapper.addClass( 'cke_image_tools_wrapper' );
            toolsWrapper.append( imageOrLink );
            toolsWrapper.append( tools );
            widget.element.append( toolsWrapper, true );
        } else {
            widget.wrapper.append( tools );
        }

        tools.on( 'click', function( evt ) {
            var image = widget.parts.image;
            var imageToolId = evt.data.getTarget().getAttribute('data-cke-image-tool-id') || evt.data.getTarget().getParent().getAttribute('data-cke-image-tool-id');
            if (!imageToolId) {
                return;
            }

            // Save the undo snapshot first: before using tool.
            ...
            case 'deflate':
                ...
                var newWidth = image.$.clientWidth/2;
                var newHeight = image.$.clientHeight/2;
                image.setAttributes( { width: newWidth, height: newWidth } );
                widget.setData( { width: newWidth, height: newHeight } );
                editor.fire( 'saveSnapshot' );
                break;
            case 'optimize':
                var newWidth = 800;
                var newHeight = image.$.clientHeight*(800/image.$.clientWidth);
                image.setAttributes( { width: newWidth, height: newWidth } );
                widget.setData( { width: newWidth, height: newHeight } );
                editor.fire( 'saveSnapshot' );
                break;
            case 'origin':
                var newWidth = image.$.natualWidth;
                var newHeight = image.$.natualWidth;
                image.setAttributes( { width: newWidth, height: newWidth } );
                widget.setData( { width: newWidth, height: newHeight } );
                editor.fire( 'saveSnapshot' );
                break;
            case 'border':
                if (image.hasClass('cke_image_border')) {
                    image.removeClass('cke_image_border')
                } else {
                    image.addClass('cke_image_border')
                }
                editor.fire( 'saveSnapshot' );
                break;
            case 'edit':
                function popupCenter(url, title, w, h) {
                    ...
                    var child = window.open(url, title, 'toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=' + w + ', height=' + h + ', top=' + top + ', left=' + left);
                ...
                swEditor = editor;
                swEditor.selectedImage = image.$;
                swEditor.insertCapturedImage = function(capturedImageData) {
                    var loader = editor.uploadRepository.create( capturedImageData );
                    loader.on( 'update', function() {
                        if (loader.status === 'uploaded' && loader.responseData && loader.responseData.url) {
                            image.setAttributes( { src: loader.responseData.url});
                            setTimeout(function(){
                                var newWidth = image.$.width;
                                var newHeight = image.$.height;
                                image.setAttributes( { width: newWidth, height: newHeight});
                                widget.setData( { src: loader.responseData.url, width: newWidth, height: newHeight } );
                                editor.fire( 'saveSnapshot' );
                            },300);
                        }
                    } );
                    ...
                    loader.loadAndUpload( editor.config.imageUploadUrl );
                };
                popupCenter("../smartimage/imageCapture.html", editor.lang.image2.smartimage, 1024, 700);
                break;
            case 'remove':
