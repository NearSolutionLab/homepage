...
( function() {
	...
	function setUrl( fileUrl, data ) {
        ...
        if (dialog && dialog._.name === 'image2') {
            $(dialog.parts.dialog.$).parent().show().prevAll('.cke_dialog_background_cover').show();
        }
		...
		// Modified by YSJUNG for smartworks.net
		if ( fileUrl ) {
	        updateTargetElement( fileUrl, this._.filebrowserSe );
	        $(dialog.parts.dialog.$).find(".cke_dialog_ui_button_ok span").click();
		}
    // Modified by YSJUNG for smartworks.net
	}
